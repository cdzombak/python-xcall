...
TEST_STRING = r""" -- () ? & ' " ‘quoted text’ _x_y_z_ a://b.c/d?e=f&g=h"""

# string -> json -> utf8 -> quote
ENCODED_TEST_STRING = urllib.parse.quote(json.dumps(TEST_STRING).encode("utf8"))
...
def test_string_coding_and_deencodin():
    assert json.loads(urllib.parse.unquote(ENCODED_TEST_STRING)) == TEST_STRING
...
def test_Popen_mocking():
    ...
    process = mock_Popen(["a", "b"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def test_xcall__no_parameters(monkeypatch):
    ...
    popen_args = mock_Popen.call_args[0][0]
    assert popen_args == [xcall.XCALL_PATH, "-url", '"scheme://x-callback-url/action"']
...
def test_xcall__with_unicode_and_unsafe_html_parameters(monkeypatch):
    ...
    popen_args = mock_Popen.call_args[0][0]
    encoded_test_string = urllib.parse.quote(TEST_STRING.encode("utf8"))
    assert popen_args == [
        xcall.XCALL_PATH,
        "-url",
        '"scheme://x-callback-url/action?key1=%s"' % encoded_test_string,
    ]
...
def test_xcall__xerror_response(monkeypatch):
    ...
    with pytest.raises(xcall.XCallbackError) as excinfo:
        xcall.xcall("scheme", "action")
    assert (
        "x-error callback: 'x-error response' (in response to url: "
